...
OS. The free OS is what it always was underneath: the doctrine (五柱 · 四谋), the Claude Code skills +
commands, and the 库 Arsenal wiki — all runnable on a Claude Pro login with no keys.

- **Removed the vendored app shell** from the repo (`w-counsel/app/`): the app is now distributed
  separately. `w-counsel/` keeps only the README + preview image that describe the upgrade.
- **Publish gate hardened** (`scripts/verify.mjs`): the repo walk no longer crashes on unreadable
  scratch dirs (`.gstack/`), and a new tracked-file hygiene check fails the gate if any browser-debug
